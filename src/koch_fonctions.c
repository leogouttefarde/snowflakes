...
void init_koch(struct list **koch, uint32_t size, uint32_t segment_length)
{
	...
	*koch = pt;
	...
	pt->x = BORDER;
	pt->y = size - BORDER;
	pt->next = malloc(sizeof(struct list));

	pt = pt->next;
	pt->x = BORDER + segment_length/2;
	pt->y = BORDER;
	pt->next = malloc(sizeof(struct list));

	pt = pt->next;
	pt->x = BORDER + segment_length;
	pt->y = size - BORDER;
	pt->next = NULL;
}
...
void generer_koch(struct list *koch, uint32_t nb_iterations)
{
	...
	if (nb_iterations > 0)
	{
		...
		while (pt)
		{
			...
			pt_b->x = (int32_t)pt->x + ((int32_t)next->x - (int32_t)pt->x)/3;
			pt_b->y = (int32_t)pt->y + ((int32_t)next->y - (int32_t)pt->y)/3;

			pt_d->x = (int32_t)pt->x + (2 * ((int32_t)next->x - (int32_t)pt->x)) / 3;
			pt_d->y = (int32_t)pt->y + (2 * ((int32_t)next->y - (int32_t)pt->y)) / 3;
